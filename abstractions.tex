...
\begin{frame}{Peak II}
A slippery concept. What follows is my opinion only.
\begin{itemize}
\item ``Peak'' is most precisely defined in terms of the processor's instruction set
(which might be irrelevant).
\item ``Peak'' is most relevantly defined in terms of our problems's best possible solution
(which might be unknown).
\item ``Peak'' is most productively defined in terms of the instructions our actual
implementation would use assuming no structural hazards.
\end{itemize}
\end{frame}
