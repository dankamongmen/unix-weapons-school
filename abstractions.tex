...
\item \textit{Intel 64 and IA-32 Architectures Optimization Manuals}. Intel.
\end{itemize}
\end{frame}
...
different $P$ approach their own peaks. Most often, we use ``peak'' to refer
...
\textbf{Implication:} for fixed work and $\mu$arch, lower frequencies cannot
 usually save power.
\end{frame}
...
\vspace{.25in}
Each time step is a \textit{cycle} ($\mathbbm{c}$). 1GHz$\implies$ 1ns $\mathbbm{c}$-time.\\
\vspace{.25in}
...
\item IPC: Instructions per $\mathbbm{c}$
\item CPI: $\mathbbm{c}$ per instruction
\end{itemize}
\end{frame}
...
\begin{enumerate}
\item Reduce $\mathbbm{c}$-time (better $\mu$architecture, better materials)
...
arithmetic/register throughput to arithmetic intensity ($\mathbbm{c}$ per word) will not be able
to sustain peak.
\end{frame}
...
execution core at full throughput.
\begin{itemize}
\item 16-byte/$\mathbbm{c}$ fetches from 32KB 8-way\footnote{4-way on Nehalem} I\$
\item Speculative decoding at each of fetch's 16 bytes
\item 1 complex decoder, 3 simple decoders
\item 32 sets of 8 ways of 6 $\mu$ops $\mu$I\$ (``decoded I\$'')
\end{itemize}
\end{frame}

\begin{frame}{Delays, or Where Have All the $\mathbbm{c}$ Gone?}
\begin{itemize}
\item Instruction fetch miss ($7\mathbbm{c}$ for ITLB miss + 2TLB hit)
\item $3\mathbbm{c}$ for each non-REX length changing prefix\footnote{$6\mathbbm{c}$ for any fetch containing an LCP on Nehalem}
