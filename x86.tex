...
twenty years\footnote{Commonly expressed as ``All the world's an x86.''}. That said, if you can chase peak on x86, you can
...
\begin{tabular}{|l|l|}
\hline
MOVNT*Q/MOVNTPS & Streaming store to memory\\
\hline
MOVNTDQA & Streaming loads from USWC MMIO (SSE4.1)\\
\hline
PREFETCHNTA & Prefetch into second-level cache\\
\hline
PREFETCHNT0 & Prefetch into all cache levels\\
\hline
MASKMOVD* & Bytemask store from register to memory\\
\hline
SFENCE & Order point for stores\\
\hline
LFENCE & Order point for loads\\
\hline
MFENCE & Order point for stores and loads\footnote{\tiny{Not equivalent to SFENCE+LFENCE, which would be mutually unordered!}}\\
\hline
WBINVD & Write back, and invalidate cache\\
\hline
INVD & Screw it, just invalidate the damn caches\\
\hline
INVLPG & Invalidate TLB entries\\
\hline
CLFLUSH & Cache line invalidation (all levels, broadcasted)\\
\hline
MONITOR/MWAIT & Monitor a cache line (SSE3)\\
\hline
\end{tabular}%
}
};
