...
electrons as charge carriers, n+ source, drain, and gate, and p-substrate. With no
mobile carriers, $I_D = 0$. $V^{+}_{GS}$
is applied to the gate, attracting electrons as $V_{GS}\ \text{exceeds the threshold
voltage} V_{TH}$, establishing a salient. When $V_{DSat}\triangleq V_{GS} - V_{TH} \ge V_{DS}$,
a conducting channel exists from source to drain, and $I_D = \frac{\mu_{n}C^{*}_{oxn}W}{2L}$.
}
\\
\vfill
