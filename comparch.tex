...
\scriptsize{Given inverters, we can build (\textit{functionally complete}) NORs and NANDs\footnote{\tiny{CMOS NOR/NAND in 4 transistors each; TTL multiemitters requires 5 for NOR vs. 4 for NAND.}}\ldots}
\end{figure}
\begin{figure}
\centering{
...
\vspace{.1in}
\scriptsize{\ldots given NORs and NANDs, we can build universal computers\ldots}
\end{figure}
...
\begin{frame}{The care and feeding of gates}
\tiny{CMOS inverter voltage transfer function}
\hfill\textit{Symmetric CMOS inverters} ($V_{T_{n}} = |V_{T_{p}}|$,
$K_n = K_p\footnote{\tiny{$K_{foo,ox} = \frac{\mu_{foo}\varepsilon_{ox}}{2T_{ox}}$,
where $\mu = \text{electron mobility}$,
$\varepsilon = \text{dielectric constant}$,
and $T = \text{insulator thickness}$.
}}$, $C^{*}_{oxn} = C^{*}_{oxp}$) having zero static draw are easily built via
combination of a pMOS and nMOS transistor.
)
\\
\includegraphics[scale=.2]{images/vtf.png}\\
\vfill
