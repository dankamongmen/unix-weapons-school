...
required to switch means less time to switch (faster devices!) means less work
done means less power drawn means less heat dissipated\footnote{\tiny{Which itself means faster, lower-power, more reliable devices.}}.}
\vfill
...
channel length L $\sim$ depletion layer widths, \textit{short-channel effects}\footnote{\tiny{DIBL,
velocity saturation, impact ionization, surface scattering,
and dreaded ``hot carrier injection''.}} come into play. Less junction material amplifies
...
from gate to channel. Voltages must be reduced to combat HCI.}
\vfill
