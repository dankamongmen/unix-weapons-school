...
we merely \textit{exploit}.% दत्त दयध्वम् दम्यत.
\end{frame}
...
physics\footnote{\tiny{``Dilating time'' (``relativistic computing?'') is some
non-starter Discovery Channel horseshit.}}. Hold on tight!
\end{frame}

\begin{frame}{\textbf{c}-time}
\scriptsize{
Taken to the limit, we can run asynchronously:
\begin{equation}
\phi = \phi_{logic}
\end{equation}
...
The number of transistors on an IC doubles every $\phi_{moore}$ (i.e.,
...
\vspace{.1in}
Good! More transistors means larger scales of integration, bigger on-die
caches, wider SIMD, etc.\ldots
\vspace{.1in}
Bad! Interconnect lines show greater resistance with reduced dimension. More
transistors, everything else being equal, mean more power draw. 
\end{frame}
